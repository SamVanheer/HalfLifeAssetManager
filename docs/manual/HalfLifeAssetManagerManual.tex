...
\item Source 1 Model Viewer: Path to a model viewer that can display Source 1 models
	...
	Known model viewers:
	\begin{itemize}
	\item Source 1 \textit{hlmv.exe} included with Source 1 games. Prefer using a model viewer running on the latest engine branch to ensure all 4 versions of the format can be opened.
	\end{itemize}
\item Studiomdl compiler: Path to a \textit{studiomdl.exe} compiler that can compile Half-Life 1 models

	Known compilers:
	\begin{itemize}
	\item \href{http://www.the303.org/backups/sven_studiomdl_2019.rar}{Sven Co-op Studiomdl Compiler}
	\item Standard compiler included with the \href{https://steamdb.info/app/254430/}{Half-Life SDK on Steam} in the \textit{Model Tools} directory.
	\end{itemize}
\item Studiomdl Decompiler: Path to a \textit{mdldec.exe} decompiler that can decompile Half-Life 1 models
...
\vspace{\baselineskip}
If a game's \textit{liblist.gam} file contains a \textit{fallback\_dir} entry it will be used to find that game's configuration and use it as well as the originating game's configuration. This matches the engine's behavior.
...
\paragraph{Dump Model Info...}

Dumps model info to a file of the user's choosing.

\paragraph{Take Screenshot...}
...
\subsubsection{Tool Menu}

The StudioModel Tool menu adds actions for interacting with StudioModel files outside the program.

\paragraph{Compile Model...}

Opens the Studiomdl compiler frontend.

\paragraph{Decompile Model...}

Opens the Studiomdl decompiler frontend.

\paragraph{Edit QC File...}

Opens the selected QC file in the system-configured text editor.

\newpage

\subsubsection{Options Dialog}

The StudioModel page is added to provide options specific to this format.

\paragraph{Automatically switch to the First Person camera for view models}

When enabled models whose name starts with \textit{v\_} are loaded with the First Person camera activated.

\paragraph{Activate the Texture view when the Textures panel is opened}
...
\paragraph{Ground Length}

The length of the ground in units.

\paragraph{Sound events}

List of event Ids that should play sounds.

The following Ids are used in Half-Life: \textit{1004}, \textit{1008}, \textit{5004}.

The following Ids are used in Opposing Force: \textit{1011}, \textit{1012}, \textit{1013}.
...
\vspace{\baselineskip}
The First Person camera shows models from the first person perspective and is useful mostly for weapon viewmodels (\textit{v\_} models).
...
\vspace{\baselineskip}
Animations are referenced by the game in a couple different ways. NPCs commonly use activities to pick an animation but may also use the name or part of it. The Human Grunt for instance selects the stance used to aim and fire weapons using a prefix like \textit{crouching\_}.

\vspace{\baselineskip}
Weapons in particular use the animation index, displayed in parentheses in the sequence list.

\vspace{\baselineskip}
A special note for the Tentacle monster (\textit{monster\_tentacle}): this NPC uses custom activities. The program recognizes these and supports the use of custom activities just like the compiler does using the syntax \textit{ACT\_$<$number$>$} where \textit{$<$number$>$} is an integer value. This value can be a negative value though the game only uses positive values.
...
\vspace{\baselineskip}
The Body Value contains the value that entities store in the \textit{body} keyvalue to enable a specific set of submodels.
...
\vspace{\baselineskip}
Remapping works by using a \textit{low}, \textit{mid} and \textit{high} color table index to replace the hue values with the selected top and bottom colors. The range \textit{low} to \textit{mid} is replaced with the top color, the range \textit{mid + 1} to \textit{high} is replaced with the bottom color.

\vspace{\baselineskip}
Color remapping only works if the texture has a specific name:

\begin{itemize}
\item If the name has the format \textit{remap$<$number$>$\_$<$low$>$\_$<$mid$>$\_$<$high$>$.bmp} then \textit{low}, \textit{mid} and \textit{high} are extracted from the texture name.
\item If the name has the format \textit{remapC\_$<$low$>$\_$<$mid$>$.bmp} (C can be lowercase c as well) then only \textit{low} and \textit{mid} are extracted.
\item If the name is \textit{DM\_Base.bmp} then \textit{low} is set to \textbf{160}, \textit{mid} is set to \textbf{191} and \textit{high} is set to \textbf{223}.
\end{itemize}

Color table indices must always contain 3 numbers, for example an index of \textbf{10} is \textbf{010}. To work properly \textit{mid} must be greater than \textit{low}. \textit{high} can be \textbf{0} to disable bottom color remapping, otherwise it must be greater than \textit{mid + 1}.
